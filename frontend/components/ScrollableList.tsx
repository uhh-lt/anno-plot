...
const ScrollableList = ({}) => {
  ...
  useEffect(() => {
    ...
    const filtered_arrows = arrows
      .filter((arrow) => filteredCodes.includes(arrow.code_id))
    ...
    const newItems = filtered_arrows.map((arrow) => {
      ...
      return {
        ...
        title:
          getPath(
            codes,
            codes.find((c) => c.code_id === arrow.code_id),
          ) +
          " (" +
          reduced_segment +
          ")",
  ...
  return (
    <div style={{ overflowY: "auto", maxHeight: "800px" }}>
      ...
      {items.map(
        (item, index) => item !== null && <ListItem key={index} item={item} onDelete={() => onDelete(item.id)} />,
      )}
    </div>
  );
};
