...
const Table = () => {
    ...
    const sendColorChange = (color, code) => {
        updateCode(currentProject, code.code_id, code.text, code.parent_code_id ,color).then(()=> {fetchCodes()})
    }
...
const handleCategoryFiltering = (event, category) => {
    ...
    const selectAllCategories = () => {
        codes.map(code => {updateFilteredCodes(code.code_id, true);})
    };
...
<div onContextMenu={(event) => handleContextMenu(event, node.code_id)}>
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '10px' }}>
        ...
        <div onClick={(e) => e.stopPropagation()}>
            <ColorPicker initialColor={hexToReactColor(node.color)}
