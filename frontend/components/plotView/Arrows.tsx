...
export const initializeArrows = (containerElement) => {
  const svg = d3.select(containerElement).append("svg").attr("width", 0).attr("height", 0);
  ...
  function series(selection) {
    selection.each(function (data, index, group) {
      ...
      const arrows = container
        ...
        .data(data)
        .join("line")
        .attr("x1", (d: any) => xScale(d.start.x))
        .attr("y1", (d: any) => yScale(d.start.y))
        .attr("x2", (d: any) => xScale(d.end.x))
        .attr("y2", (d: any) => yScale(d.end.y))
        .attr("stroke", (d: any) => idToColorMapper[d.code_id])
        .attr("stroke-width", 3)
        .attr("marker-end", (d: any) => `url(#arrowhead-${d.code_id})`);

      container
        .selectAll("circle.drag-handle")
        .data(data)
        .join("circle")
